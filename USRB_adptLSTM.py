...
def allvar(training_set,var_list,tau,H,num_epochs,fr,inputvar,invar_list,learning_rate,header,seq_length,num_classes,device,mgate,fluxgate,combgate,fold,static,resample,gi,g,eps,epsx):
    ...
    training_seti=training_set[1:,:]  #tar, Q, dependent variablei
    ...
    for hidden_size in tqdm(H):
        ...
        if __name__ == '__main__' :
            ...
            seq.float ()
            criterion = nn.MSELoss().to(device)     #changed to L1Loss for prediction from predicted values
            ...
            train_trfmp=utils.lencorr1(tr,ylen)
            train_trfmo=utils.lencorr1(target,ylen)
            test_trfmp=utils.lencorr1(yy,ylen)
            test_trfmo=utils.lencorr1(test_target,ylen)
...
fluxgate=['yes','no']
fluxgateid=1 #flux gate
method=['regLSTM','mLSTM(tanh)']
ind=0         #flow gate
combgateid=0  #adaptive gate
...
H = [25,30,32,40,45,50,55,60,64,70,75,80,85,90,100,128]
...
address='/home/.../monticello_'+resfilename[res]  #data address
    ...
    for epx in range(len(epsfx)):
        ...
        for k1 in range(len(fol)):
            for i in range (len (var)) :
                ...
                st=np.ones((len(u),len(stval)))


                for k in range(len(seq_length)):
                    process1 = Process (target=allvar, args=(u,var_list[i], nk, H, num_epochs, fr[0], inputvar1[i],
